...
function logAdvancedTest(category, name, status, message = '') {
  ...
  console.log(`${icon} [${category.toUpperCase()}] ${name}: ${status}${message ? ` - ${message}` : ''}`);
...
function checkBundleOptimization() {
  ...
  files.forEach(file => {
    if (fs.existsSync(file)) {
      ...
      const hasLazyLoading = content.includes('React.lazy') || content.includes('dynamic import');
      
      // Check for unnecessary re-renders
      const hasUseMemo = content.includes('useMemo') || content.includes('useCallback');
      
      // Check for proper key props in lists
      const hasKeyExtractor = content.includes('keyExtractor') || content.includes('key=');
      
      if (hasKeyExtractor) {
        logAdvancedTest('performance', `List optimization in ${path.basename(file)}`, 'PASS');
      } else {
        logAdvancedTest('performance', `List optimization in ${path.basename(file)}`, 'FAIL', 'Missing keyExtractor or key props');
      }
      
      if (hasUseMemo) {
        logAdvancedTest('performance', `Memoization in ${path.basename(file)}`, 'PASS');
      } else {
        logAdvancedTest('performance', `Memoization in ${path.basename(file)}`, 'WARN', 'Consider adding useMemo/useCallback');
      }
    }
  });
}
...
function checkSecurityPractices() {
  ...
  if (fs.existsSync(firebaseConfig)) {
    ...
    if (hasEnvVars) {
      ...
      logAdvancedTest('security', 'Environment variables usage', 'WARN', 'Consider using env vars for sensitive data');
    }
    
    // Check for API key exposure
    const hasApiKeyFallback = content.includes('AIzaSy');
    if (hasApiKeyFallback) {
      logAdvancedTest('security', 'API key security', 'WARN', 'Fallback API key detected - ensure env vars are used in production');
    } else {
  ...
  if (fs.existsSync(chatFile)) {
    ...
    const hasInputValidation = content.includes('trim()') && content.includes('maxLength');
    
    if (hasInputValidation) {
      logAdvancedTest('security', 'Input validation in chat', 'PASS');
    } else {
      logAdvancedTest('security', 'Input validation in chat', 'FAIL', 'Missing input validation');
    }
  }
}
...
function checkUXPatterns() {
  ...
  const files = ['app/(tabs)/index.tsx', 'app/new-chat.tsx', 'app/chat/[id].tsx'];
  ...
  files.forEach(file => {
    if (fs.existsSync(file)) {
      ...
      const hasLoadingState = content.includes('ActivityIndicator') || content.includes('loading');
      if (hasLoadingState) {
        logAdvancedTest('ux', `Loading states in ${path.basename(file)}`, 'PASS');
      } else {
        logAdvancedTest('ux', `Loading states in ${path.basename(file)}`, 'FAIL', 'Missing loading indicators');
      }
      
      // Check for error handling
      const hasErrorHandling = content.includes('try') && content.includes('catch');
      if (hasErrorHandling) {
        logAdvancedTest('ux', `Error handling in ${path.basename(file)}`, 'PASS');
      } else {
        logAdvancedTest('ux', `Error handling in ${path.basename(file)}`, 'WARN', 'Consider adding error handling');
      }
      
      // Check for empty states
      const hasEmptyState = content.includes('EmptyState') || content.includes('No ') || content.includes('empty');
      if (hasEmptyState) {
        logAdvancedTest('ux', `Empty states in ${path.basename(file)}`, 'PASS');
      } else {
        logAdvancedTest('ux', `Empty states in ${path.basename(file)}`, 'WARN', 'Consider adding empty state handling');
      }
    }
  });
}
...
function checkAccessibility() {
  ...
  if (fs.existsSync(accessibilityHook)) {
    ...
    if (hasScreenReader) {
      ...
      logAdvancedTest('accessibility', 'Screen reader support', 'FAIL', 'Missing screen reader support');
    }
    
    if (hasReduceMotion) {
      logAdvancedTest('accessibility', 'Reduce motion support', 'PASS');
    } else {
      logAdvancedTest('accessibility', 'Reduce motion support', 'FAIL', 'Missing reduce motion support');
    }
    
    if (hasFontScaling) {
      logAdvancedTest('accessibility', 'Font scaling support', 'PASS');
    } else {
      logAdvancedTest('accessibility', 'Font scaling support', 'FAIL', 'Missing font scaling support');
    }
  }
  ...
  if (fs.existsSync(chatFile)) {
    const content = fs.readFileSync(chatFile, 'utf8');
    const hasAccessibilityLabels = content.includes('accessibilityLabel') || content.includes('accessibilityHint');
    
    if (hasAccessibilityLabels) {
      logAdvancedTest('accessibility', 'Accessibility labels in chat', 'PASS');
    } else {
      logAdvancedTest('accessibility', 'Accessibility labels in chat', 'WARN', 'Consider adding accessibility labels');
    }
  }
}
...
function checkCodeOptimization() {
  ...
  const tsFiles = ['app/_layout.tsx', 'src/redux/store.ts', 'src/services/firebaseSimple.ts'];
  ...
  tsFiles.forEach(file => {
    if (fs.existsSync(file)) {
      ...
      if (hasInterfaces) {
        logAdvancedTest('optimization', `TypeScript types in ${path.basename(file)}`, 'PASS');
      } else {
        logAdvancedTest('optimization', `TypeScript types in ${path.basename(file)}`, 'WARN', 'Consider adding more type definitions');
      }
    }
  });
  ...
  componentFiles.forEach(file => {
    if (fs.existsSync(file)) {
      ...
      if (hasCleanImports) {
        logAdvancedTest('optimization', `Import optimization in ${path.basename(file)}`, 'PASS');
      } else {
        logAdvancedTest('optimization', `Import optimization in ${path.basename(file)}`, 'WARN', 'Consider reviewing imports');
      }
    }
  });
}
...
function checkMobileOptimizations() {
  ...
  if (fs.existsSync(responsiveFile)) {
    ...
    const hasBreakpoints = content.includes('breakpoints') || content.includes('isSmallDevice');
    const hasResponsiveFonts = content.includes('fontScale') || content.includes('fontSize');
    const hasResponsiveSpacing = content.includes('spacing') || content.includes('wp') || content.includes('hp');
    
    if (hasBreakpoints) {
      logAdvancedTest('optimization', 'Device breakpoints', 'PASS');
    } else {
      logAdvancedTest('optimization', 'Device breakpoints', 'FAIL', 'Missing device breakpoints');
    }
    
    if (hasResponsiveFonts) {
      logAdvancedTest('optimization', 'Responsive typography', 'PASS');
    } else {
      logAdvancedTest('optimization', 'Responsive typography', 'FAIL', 'Missing responsive fonts');
    }
    
    if (hasResponsiveSpacing) {
      logAdvancedTest('optimization', 'Responsive spacing', 'PASS');
    } else {
      logAdvancedTest('optimization', 'Responsive spacing', 'FAIL', 'Missing responsive spacing');
    }
  }
  ...
  if (fs.existsSync(chatFile)) {
    ...
    const hasKeyboardListeners = content.includes('Keyboard.addListener') || content.includes('keyboardWillShow');
    
    if (hasKeyboardAvoidingView) {
      logAdvancedTest('optimization', 'Keyboard avoidance', 'PASS');
    } else {
      logAdvancedTest('optimization', 'Keyboard avoidance', 'FAIL', 'Missing KeyboardAvoidingView');
    }
    
    if (hasKeyboardListeners) {
      logAdvancedTest('optimization', 'Keyboard event handling', 'PASS');
    } else {
      logAdvancedTest('optimization', 'Keyboard event handling', 'WARN', 'Consider adding keyboard listeners');
    }
  }
}
...
console.log('\n' + '='.repeat(60));
console.log(`🎯 OVERALL SCORE: ${overallPercentage}% (${totalPassed}/${totalPassed + totalFailed})`);
