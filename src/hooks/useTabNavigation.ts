...
export const useTabNavigation = (config: TabNavigationConfig = {}) => {
  ...
  const navigateToTab = (targetIndex: number, animated: boolean = true) => {
    ...
    if (targetIndex === currentIndex || targetIndex < 0 || targetIndex >= tabRoutes.length) {
      return false;
    }
  ...
  const startAutoSwipe = (interval: number = 3000, direction: 'forward' | 'backward' = 'forward') => {
  ...
  const handleSwipeGesture = (direction: 'left' | 'right', velocity?: number) => {
