...
async function validateMobileOnlySetup() {
  ...
  runTest('No Web Build Directories', () => {
    const webDirs = ['public', 'web-build', 'dist', '.expo/web', 'build', 'out'];
    let webDirsFound = [];
  ...
  runTest('No Web-Specific Imports in Source Code', () => {
    ...
    sourceFiles.forEach(file => {
      if (fs.existsSync(file)) {
        ...
        if (webWindowPattern.test(content) && !reactNativeWindowPattern.test(content)) {
  ...
  if (testResults.failed === 0) {
    console.log('🟢 ALL VALIDATION TESTS PASSED - Your project is perfectly mobile-only!');
  } else {
