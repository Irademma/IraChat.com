...
export default function SwipeableTabWrapper({ children }: SwipeableTabWrapperProps) {
  ...
  const onGestureEvent = (event: any) => {
    ...
    translateX.setValue(Math.max(-maxTranslation, Math.min(maxTranslation, tx)));
  };
  ...
  const onHandlerStateChange = (event: any) => {
    ...
    if (event.nativeEvent.state === State.END || event.nativeEvent.state === State.CANCELLED) {
      ...
      if ((translationX > threshold || velocityX > 800) && currentTabIndex > 0) {
        // Swipe right - go to previous tab (only if not at first tab)
        newTabIndex = currentTabIndex - 1;
      } else if ((translationX < -threshold || velocityX < -800) && currentTabIndex < tabRoutes.length - 1) {
  ...
  const gestureConfig = {
    ...
    activeOffsetX: currentIndex === 0 ? [-20, 0] :
                  currentIndex === tabRoutes.length - 1 ? [0, 20] :
                  [-20, 20],
    failOffsetY: [-40, 40],
