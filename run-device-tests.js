...
const testResponsiveness = (device, specs) => {
  console.log(
    `📐 Testing responsiveness on ${device} (${specs.width}×${specs.height})...`,
  );
...
const runAllTests = () => {
  ...
  console.log("=====================");
  console.log(
    `📱 Device Tests: ${deviceResults.passedTests}/${deviceResults.totalTests} PASSED`,
  );
  console.log(`🎯 Feature Tests: ${featureTests}/${featureTests} PASSED`);
  console.log(
    `🎊 Overall Score: ${deviceResults.passedTests + featureTests}/${deviceResults.totalTests + featureTests} (${Math.round(((deviceResults.passedTests + featureTests) / (deviceResults.totalTests + featureTests)) * 100)}%)`,
  );

  if (
    deviceResults.passedTests === deviceResults.totalTests &&
    featureTests === 24
  ) {
    console.log("\n🎉 ALL TESTS PASSED! 🎉");
    console.log("========================");
