...
async function testResponsivePerfection() {
  ...
  testPerfection("responsive", "Responsive Utilities Available", () => {
    ...
    const hasDeviceDetection =
      content.includes("isVerySmallDevice") &&
      content.includes("isSmallDevice");
...
async function generatePerfectionReport() {
  ...
  // Overall score
  const overallScore =
    perfectionResults.overall.total > 0
      ? Math.round(
          (perfectionResults.overall.passed / perfectionResults.overall.total) *
            100,
        )
      : 100;
  ...
  console.log(
    `✅ ${perfectionResults.overall.passed}/${perfectionResults.overall.total} tests passed`,
  );
