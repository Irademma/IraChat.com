...
import {
  View, Text, TextInput, TouchableOpacity, FlatList, KeyboardAvoidingView, Platform, Keyboard, Dimensions
} from 'react-native';
...
export default function ChatRoomScreen() {
  ...
  useEffect(() => {
    const keyboardWillShow = (event: any) => {
      console.log('🎹 [ReactNav] Keyboard will show:', event.endCoordinates.height);
    ...
    const keyboardDidShow = (event: any) => {
      console.log('🎹 [ReactNav] Keyboard did show:', event.endCoordinates.height);
      setKeyboardHeight(event.endCoordinates.height);
      setIsKeyboardVisible(true);
    };
  ...
  const renderItem = ({ item }: any) => {
    ...
    return (
      <View className={`mx-4 my-2 max-w-[80%] ${isMyMessage ? 'self-end' : 'self-start'}`}>
        {!isMyMessage && (
        ...
        <View style={{ flexDirection: 'row', alignItems: 'flex-end' }}>
          ...
          >
            <TextInput
              ...
              maxLength={1000}
              onFocus={() => {
                console.log('🎯 [ReactNav] Input focused - scrolling to bottom');
                setTimeout(() => {
                  flatListRef.current?.scrollToEnd({ animated: false });
                }, 300);
              }}
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.8}
            accessibilityLabel={input.trim() ? "Send message" : "Enter text to send"}
