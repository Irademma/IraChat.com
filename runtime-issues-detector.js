...
try {
  ...
  criticalDeps.forEach(dep => {
    if (!deps[dep]) {
      addIssue('critical', 'Missing Dependency', `${dep} is not installed`, 
  ...
  if (deps['react-native'] && deps['expo']) {
    ...
    addIssue('minor', 'Version Compatibility', 'Should verify React Native and Expo compatibility',
      'May cause build issues or runtime errors',
      'Check Expo SDK compatibility matrix');
  }
} catch (error) {
  addIssue('critical', 'Package.json Error', 'Cannot read package.json',
    'Build system will fail', 'Fix package.json syntax');
}
...
try {
  ...
  if (!hasTypes) {
    addIssue('major', 'Missing Types', 'No TypeScript type definitions found',
  ...
  tsFiles.forEach(file => {
    if (fs.existsSync(file)) {
      ...
      if (anyMatches && anyMatches.length > 3) {
        addIssue('minor', 'Type Safety', `${file} has many 'any' types`,
      ...
      if (content.includes('useState') && !content.includes('from \'react\'') && !content.includes('from "react"')) {
        addIssue('critical', 'Missing Import', `${file} uses useState without importing`,
          'App will crash on load', 'Add proper React imports');
      }
    }
  });
} catch (error) {
  addIssue('major', 'TypeScript Check Error', 'Failed to analyze TypeScript',
    'Cannot verify type safety', 'Check TypeScript configuration');
}
...
try {
  ...
  if (fs.existsSync(firebaseFile)) {
    ...
    if (content.includes('your-project-id') || content.includes('placeholder') || 
        content.includes('demo-project') || content.includes('your-api-key')) {
      addIssue('critical', 'Firebase Config', 'Firebase has placeholder configuration',
    ...
    if (!content.includes('getFirestore')) {
      addIssue('critical', 'Firestore Missing', 'Firestore not initialized',
        'Database operations will fail', 'Add Firestore initialization');
    }
    
    if (!content.includes('getStorage')) {
      addIssue('major', 'Storage Missing', 'Firebase Storage not initialized',
        'Media uploads will fail', 'Add Firebase Storage initialization');
    }
  } else {
    addIssue('critical', 'Firebase Missing', 'Firebase configuration file missing',
      'No backend connectivity', 'Create Firebase configuration');
  }
} catch (error) {
  addIssue('critical', 'Firebase Error', 'Firebase configuration has errors',
    'Backend will not work', 'Fix Firebase setup');
}
...
try {
  ...
  requiredRoutes.forEach(route => {
    if (!fs.existsSync(route)) {
      addIssue('critical', 'Missing Route', `Route ${route} is missing`,
  ...
  if (fs.existsSync(tabLayout)) {
    ...
    if (!content.includes('Tabs.Screen')) {
      addIssue('critical', 'Tab Navigation', 'Tab layout has no screens defined',
        'Tab navigation will not work', 'Add Tabs.Screen components');
    }
  }
} catch (error) {
  addIssue('critical', 'Navigation Error', 'Navigation analysis failed',
    'Cannot verify navigation setup', 'Check navigation configuration');
}
...
try {
  ...
  if (fs.existsSync(storeFile)) {
    ...
    if (!content.includes('userReducer') && !content.includes('user:')) {
      addIssue('critical', 'User Reducer Missing', 'User state management missing',
        'User authentication and data will not work',
        'Add user reducer to store');
    }
    
    if (!content.includes('chatReducer') && !content.includes('chat:')) {
      addIssue('major', 'Chat Reducer Missing', 'Chat state management missing',
        'Chat functionality may not work properly',
        'Add chat reducer to store');
    }
  } else {
    addIssue('critical', 'Redux Store Missing', 'Redux store not configured',
      'App state management will fail', 'Create Redux store configuration');
  }
  ...
  if (!fs.existsSync(userSlice)) {
    addIssue('critical', 'User Slice Missing', 'User Redux slice missing',
      'User state management will fail', 'Create userSlice.ts');
  }
  
  if (!fs.existsSync(chatSlice)) {
    addIssue('major', 'Chat Slice Missing', 'Chat Redux slice missing',
      'Chat state management will fail', 'Create chatSlice.ts');
  }
} catch (error) {
  addIssue('critical', 'Redux Error', 'Redux store analysis failed',
    'Cannot verify state management', 'Check Redux configuration');
}
...
try {
  ...
  const services = [
    { file: 'src/services/authService.ts', name: 'Authentication', critical: true },
    { file: 'src/services/chatService.ts', name: 'Chat', critical: true },
    { file: 'src/services/mediaService.ts', name: 'Media Upload', critical: false },
    { file: 'src/services/contactsService.ts', name: 'Contacts', critical: false },
    { file: 'src/services/callingService.ts', name: 'Calling', critical: false }
  ];
  
  services.forEach(service => {
    if (!fs.existsSync(service.file)) {
      addIssue(service.critical ? 'critical' : 'major', 
        'Missing Service', `${service.name} service missing`,
      ...
      if (content.length < 500) {
        addIssue('major', 'Incomplete Service', `${service.name} service is too basic`,
  ...
  addIssue('major', 'Services Error', 'Service analysis failed',
    'Cannot verify service implementations', 'Check service files');
}
...
try {
  ...
  components.forEach(component => {
    if (!fs.existsSync(component)) {
      addIssue('major', 'Missing Component', `Component ${path.basename(component)} missing`,
        'App may crash or have poor UX', `Create ${component}`);
    }
  });
} catch (error) {
  addIssue('minor', 'Component Error', 'Component analysis failed',
    'Cannot verify component implementations', 'Check component files');
}
...
if (criticalIssues.length > 0) {
  ...
  console.log(`🚨 ${criticalIssues.length} critical issues will cause app crashes`);
} else if (majorIssues.length > 3) {
  console.log('⚠️ NOT READY FOR PRODUCTION');
  console.log(`🔧 ${majorIssues.length} major issues will break core functionality`);
} else if (majorIssues.length > 0) {
  console.log('⚠️ BETA READY ONLY');
  console.log(`🧪 ${majorIssues.length} major issues need fixing for production`);
