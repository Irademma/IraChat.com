...
export const trackMessage = (type: 'text' | 'voice' | 'media', isGroup: boolean = false) => {
  if (!analytics) return;
...
export const trackGroupActivity = (action: 'created' | 'joined' | 'left', groupId: string) => {
  if (!analytics) return;
...
export const trackCall = (type: 'voice' | 'video', duration?: number) => {
  ...
  try {
    const event = type === 'voice' ? AnalyticsEvents.VOICE_CALL_STARTED : AnalyticsEvents.VIDEO_CALL_STARTED;
...
// Track swipe actions
export const trackSwipeAction = (action: 'reply' | 'archive', messageType: string) => {
  if (!analytics) return;
...
export const trackMediaInteraction = (action: 'view' | 'download' | 'share', mediaType: string) => {
  if (!analytics) return;
