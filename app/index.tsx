...
import { isFirstLaunch, markAppLaunched } from '../src/services/authStorageSimple';
...
export default function Index() {
  ...
  // Add aggressive timeout for auth initialization
  useEffect(() => {
  ...
  // Add general loading timeout fallback
  useEffect(() => {
  ...
  if (isLoading || isInitializing) {
    return (
      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: '#667eea' }}>
        ...
        <Text style={{ color: '#E2E8F0', fontSize: 14, marginTop: 8, textAlign: 'center' }}>
          {debugInfo}
        </Text>
        {__DEV__ && (
          <Text style={{ color: '#E2E8F0', fontSize: 12, marginTop: 16, textAlign: 'center' }}>
            Debug: Auth Init: {isInitializing ? 'Yes' : 'No'} | Authenticated: {isAuthenticated ? 'Yes' : 'No'}
          </Text>
        )}
      </View>
    );
