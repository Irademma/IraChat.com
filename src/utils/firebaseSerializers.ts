...
export const toFirestoreTimestamp = (timestamp: any): Timestamp => {
  ...
  // Fallback to current time
  console.warn('Unknown timestamp format for Firestore, using current time:', timestamp);
  return Timestamp.now();
};
...
export const serializeChat = (chat: any): any => {
  return {
    ...chat,
    lastMessageAt: chat.lastMessageAt ? serializeTimestamp(chat.lastMessageAt) : undefined,
...
export const deserializeChat = (chat: any): any => {
  return {
    ...chat,
    lastMessageAt: chat.lastMessageAt ? deserializeTimestamp(chat.lastMessageAt) : undefined,
