...
async function testIOSFirebaseConfiguration() {
  ...
  runTest('Firebase Plugins Configuration', () => {
    ...
    const hasFirebaseAuth = plugins.some(plugin => 
    ...
    if (hasFirebaseApp && hasFirebaseAuth) {
      ...
      if (!hasFirebaseApp) console.log('      - Missing @react-native-firebase/app');
      if (!hasFirebaseAuth) console.log('      - Missing @react-native-firebase/auth');
      return false;
    }
  });
  ...
  runTest('iOS useFrameworks Configuration', () => {
    ...
    const firebaseAuthPlugin = plugins.find(plugin => 
      Array.isArray(plugin) && plugin[0] === '@react-native-firebase/auth'
    );

    if (firebaseAuthPlugin && firebaseAuthPlugin[1] && firebaseAuthPlugin[1].ios && firebaseAuthPlugin[1].ios.useFrameworks === 'static') {
